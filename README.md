...
⭐ **Disponible para proyectos en remoto / Traslado (Nacional e Internacional)**  
💼 **Inglés Comercial e Interlocución Técnica (Nivel B2 Certificado)**
...
*   **Arquitectura:** Estructura Multi-Tenant con aislamiento de datos y control de acceso basado en roles (RBAC) mediante **Spring Security y JWT**.
*   **Frontend:** App interactiva construida con **Next.js (App Router)**, **React 19** y TypeScript estricto.
*   **Optimización de Base de Datos (PostgreSQL):** 
    *   Eliminación de **queries N+1** mediante `FetchType.LAZY` y sentencias `JOIN FETCH`.
    *   Prevención de saturación del pool desactivando `spring.jpa.open-in-view`.
    *   Indexación estratégica con `@Index` en campos relacionales clave (`tenant_id`, `user_id`).
